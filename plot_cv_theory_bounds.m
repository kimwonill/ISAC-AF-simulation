...
plot_style = struct( ...
    ...
    'legend_position', [0.135 0.325 0.172 0.150], ...
...
plot(ax1, cv, 10*log10(U), '-d', ...
     ...
     'MarkerSize', 9.4, 'MarkerFaceColor', upper_color, ...
     'Color', upper_color, ...
     'DisplayName', 'Upper');
plot(ax1, cv, 10*log10(L), '-d', ...
     'LineWidth', 2.2, 'MarkerIndices', marker_idx, ...
     'MarkerSize', 9.4, 'MarkerFaceColor', lower_color, ...
     'Color', lower_color, ...
     'DisplayName', 'Lower');
if ~isempty(sim_cv)
    scatter(ax1, sim_cv, sim_pslr_dB, 56, ...
...
set(ax1, 'FontSize', plot_style.axes_font, 'XLim', [-0.05 1.05], ...
         'XTick', 0:0.5:1, ...
...
[cv_grid, ~] = ndgrid(CV_max_list(:), 1:size(pslr_lin_grid, 2));
sim_cv = cv_grid(:);
...
function draw_pslr_legend(fig, position, upper_color, lower_color, sim_color, sim_edge_color, style, cfg)
...
legend_font = max(style.legend_font - 5, 1);
y_pos = [0.755 0.500 0.245];
labels = {'Upper', 'Lower', 'Simulation'};
