...
plot_style = struct( ...
    ...
    'axes_position', [0.16 0.15 0.81 0.74], ...
    'axes_font', 15, ...
    'label_font', 19, ...
    'title_font', 20, ...
    'legend_font', 16.5);
...
for idx = 1:numel(pdes_values)
    ...
    plot(scan_deg, beam_power_dB(:, idx), '-o', ...
         'LineWidth', 2.0, ...
         'MarkerSize', 7.0, ...
...
for l = 1:numel(target_deg)
    xline(target_deg(l), '--k', 'LineWidth', 1.6, ...
...
title_handle = title(ax, 'Beam Power vs. P_{des}', ...
                     'Interpreter', 'tex', 'FontSize', plot_style.title_font);
xlim([min(scan_deg), max(scan_deg)]);
xticks(-90:30:90);

valid_beam = beam_power_dB(isfinite(beam_power_dB));
if ~isempty(valid_beam)
    y_span = max(valid_beam) - min(valid_beam);
    if y_span <= 0
        y_span = max(abs(valid_beam(1))*0.05, 1);
    end
    ylim([min(valid_beam) - 0.04*y_span, max(valid_beam) + 0.05*y_span]);
end
...
exportgraphics(fig, png_path, 'Resolution', export_resolution);
...
if exist(paper_fig_dir, 'dir') == 7
    ...
    exportgraphics(fig, fullfile(paper_fig_dir, 'beamgain_pdes_sweep.pdf'), ...
                   'ContentType', 'image', 'Resolution', export_resolution);
end

save(fullfile(out_dir, 'beamgain_pdes_sweep_results.mat'), ...
