...
num_configurations = size(configs, 1);
archive_version = 2;
output_dir = fullfile(sim_dir, 'results', sprintf( ...
    'direct_covariance_archive_MC%d', num_mc));

H_grid = cell(1, num_configurations);
W_sdp_grid = cell(0, 0, num_configurations);
alpha_grid = cell(0, 0, num_configurations);
solver_feasible = [];
solver_status = strings(0, 0, num_configurations);
pslr_min_grid = [];
params_by_configuration = cell(1, num_configurations);
threshold_sources = cell(1, num_configurations);
configurations = repmat(struct('index', [], 'NT', [], 'N', []), ...
    1, num_configurations);
missing = strings(0, 1);

for config_index = 1:num_configurations
    ...
    if config_index == 1
        CV_max_list = cv_grid;
        W_sdp_grid = cell(num_cv, num_mc, num_configurations);
        alpha_grid = cell(num_cv, num_mc, num_configurations);
        solver_feasible = false(num_cv, num_mc, num_configurations);
        solver_status = strings(num_cv, num_mc, num_configurations);
        pslr_min_grid = nan(num_cv, num_mc, num_configurations);
    elseif ~isequal(cv_grid, CV_max_list)
        error('CV grid mismatch in %s.', source_file);
    end
    params = source.params;
    params.cvx_solver = 'mosek';
    params.cvx_solver_threads = 1;
    params_by_configuration{config_index} = params;
    threshold_sources{config_index} = threshold_source;
    configurations(config_index).index = config_index;
    configurations(config_index).NT = NT;
    configurations(config_index).N = N;
    point_dir = fullfile(output_dir, sprintf('points_NT%d_N%d', NT, N));
    ...
    for mc = 1:num_mc
        ...
        for cv_index = 1:num_cv
            point_file = fullfile(point_dir, sprintf( ...
            ...
            saved = load(point_file, 'W', 'alpha', 'H', ...
                'solver_feasible', 'solver_status', 'channel_seed', ...
                'cv_index', 'CV_max', 'pslr_min', 'archive_version');
            ...
            if ~isequal(size(saved.H), [NT, params.K, N])
            ...
            elseif ~isequal(H_by_mc{mc}, saved.H)
                error(['Channel mismatch across CV points for NT=%d, ' ...
                    'N=%d, MC=%d.'], NT, N, mc);
            end
            W_sdp_grid{cv_index, mc, config_index} = saved.W;
            alpha_grid{cv_index, mc, config_index} = saved.alpha;
            solver_feasible(cv_index, mc, config_index) = ...
                saved.solver_feasible;
            solver_status(cv_index, mc, config_index) = ...
                string(saved.solver_status);
            pslr_min_grid(cv_index, mc, config_index) = saved.pslr_min;
        end
    end
    H_grid{config_index} = cat(4, H_by_mc{:});
end

if ~isempty(missing)
    error('Archive is incomplete: %d/%d point files are missing.', ...
        numel(missing), num_configurations * num_mc * num_cv);
end

channel_seeds = 1:num_mc;
seed_policy = 'rng(mc, ''twister'')';
point_file_pattern = [ ...
    'points_NT%d_N%d/direct_covariance_MC%03d_CV%02d.mat'];

archive.archive_version = archive_version;
archive.num_mc = num_mc;
archive.num_configurations = num_configurations;
archive.configurations = configurations;
...
archive.params_by_configuration = params_by_configuration;
archive.seed_policy = seed_policy;
archive.threshold_sources = threshold_sources;
...
save(temporary_file, 'H_grid', 'W_sdp_grid', 'alpha_grid', ...
    ...
    'num_mc', 'num_configurations', 'configurations', 'channel_seeds', ...
    'params_by_configuration', 'archive_version', 'seed_policy', ...
    'threshold_sources', 'point_file_pattern', '-v7.3');
movefile(temporary_file, archive_path, 'f');

fprintf('Direct covariance archive complete: %d/%d files.\n', ...
    num_configurations * num_mc * num_cv, ...
    num_configurations * num_mc * num_cv);
fprintf('Covariance solver feasible: %d/%d (%.2f%%).\n', ...
    nnz(solver_feasible), num_configurations * num_mc * num_cv, ...
    100 * nnz(solver_feasible) / ...
        (num_configurations * num_mc * num_cv));
